...
// CPUAdapter.kt
class CPUAdapter(private val cpuList: List<CPU>) : RecyclerView.Adapter<CPUAdapter.ViewHolder>() {

    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        val tvName: TextView = view.findViewById(R.id.CPUName)
        // Добавьте переменные для brand, model, price, socket
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            openUrlInApp(holder.itemView.context, cpu.url)
        }
